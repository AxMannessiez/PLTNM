...
class SpotifyAccountApi {
        ...
        try {
            const response = await axios.post(
                ...
                qs.stringify({
                    ...
                    redirect_uri: this.#redirectUri,
                    client_id: this.#clientId,
                    code_verifier: localStorage.getItem(this.#localStorageKeys.codeVerifier)
                }),
                {
                    headers: {
    ...
    static storeToken(accessToken, expiresIn, refreshToken){
        ...
        localStorage.setItem(this.#localStorageKeys.accessTokenExpiresIn, Math.floor(Date.now() / 1000) + expiresIn);
        localStorage.setItem(this.#localStorageKeys.accessTokenRefresh, refreshToken);
    }

    static getToken() {
        return {
            accessToken: localStorage.getItem(this.#localStorageKeys.accessToken),
            expiresIn: localStorage.getItem(this.#localStorageKeys.accessTokenExpiresIn),
            refreshToken: localStorage.getItem(this.#localStorageKeys.accessTokenRefresh)
        };
    }
}

export {SpotifyAccountApi};